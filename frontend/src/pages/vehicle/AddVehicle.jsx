...
const AddVehicle = () => {
  ...
  const regex = /^[A-Z]{2}[-\s]?\d{1,2}[-\s]?[A-Z]{1,2}[-\s]?\d{4}$/;

  if (!regex.test(vehicleNumber.toUpperCase())) {
    setError("Invalid vehicle number. Please enter a valid Indian vehicle number.");
    return false;
  }
  ...
  return (
    <div className='p-4 text-[#383838]  justify-center lg:px-96'>
        ...
        <form onSubmit={sendData} encType='multipart/form-data'>
          ...
          <input type='text' list='city' className='border rounded-lg w-full p-2 mb-6 mt-2' placeholder='Delhi' onChange={(e) => setLocation(e.target.value) } required/>
